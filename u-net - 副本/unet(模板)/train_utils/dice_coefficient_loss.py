...
def dice_coeff(x: torch.Tensor, target: torch.Tensor, ignore_index: int = -100, epsilon=1e-6):
    ...
    for i in range(batch_size):
        ...
        if sets_sum == 0: # 说明x_i、t_i中元素都等于0，此时相当于都预测正确，因此此时的dice系数应当为1
            sets_sum = 2 * inter
...
def dice_loss(x: torch.Tensor, target: torch.Tensor, multiclass: bool = False, ignore_index: int = -100):
    # 此时仅仅是针对训练的时候的dice_loss的计算，在验证过程中的dice_loss与其计算的方式是不同的
...
if __name__ == '__main__':
    ...
    dice_label = build_target(label, 2, 255)
    model_out = torch.randn(size=(1,2,3,3))

    l = dice_loss(model_out,dice_label,True,ignore_index)
    print(l)

    # 交叉熵函数
    loss_weight = torch.tensor([1,2],dtype=torch.float32)
    loss = nn.functional.cross_entropy(model_out, label, ignore_index=ignore_index, weight=loss_weight)
    print(loss)
