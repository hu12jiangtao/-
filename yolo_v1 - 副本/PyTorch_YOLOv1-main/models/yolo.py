...
from models.basic import Conv, SPP
...
class myYOLO(nn.Module):
    def __init__(self, device, input_size=None, num_classes=20, trainable=False, conf_thresh=0.01, nms_thresh=0.5):
        ...
        # backbone: resnet18
        # self.backbone, feat_dim = build_resnet('resnet18', pretrained=trainable) # 源代码中的
    ...
    def decode_boxes(self, pred):
        ...
        """
        # tx、ty 相当于 对于这个预测框所在的grid_cell的左上角的坐标来说的相对位置
        # tw、th 为预测框的绝对宽高的对数值
        output = torch.zeros_like(pred)
        # 得到所有bbox 的中心点坐标和宽高
        pred[..., :2] = torch.sigmoid(pred[..., :2]) + self.grid_cell # 所有grid_cell中的bbox的中心点相对于整张图片的相对坐标
        pred[..., 2:] = torch.exp(pred[..., 2:]) # bbox的宽高

        # 将所有bbox的中心带你坐标和宽高换算成x1y1x2y2形式
        output[..., :2] = pred[..., :2] * self.stride - pred[..., 2:] * 0.5
        output[..., 2:] = pred[..., :2] * self.stride + pred[..., 2:] * 0.5

        return output # bbox左上角的绝对坐标和右下角的绝对坐标
    ...
    def postprocess(self, bboxes, scores):
        ...
        """
        # 得到最大概率的标签
        labels = np.argmax(scores, axis=1) # [W * H, ]
        # 取出所有的grid_cell中的类别最大的概率的分数
        scores = scores[(np.arange(scores.shape[0]), labels)] # [W * H, ]

        # threshold
        keep = np.where(scores >= self.conf_thresh)  # 此时返回一个列表，列表中存储着满足scores >= self.conf_thresh的索引
        # 取出所有置信度大于所设置的阈值的分数、bbox、标签
        bboxes = bboxes[keep]
        scores = scores[keep]
        labels = labels[keep]
        ...
        for i in range(self.num_classes):
            inds = np.where(labels == i)[0]  # 找出所有当前类的索引inds为一个一维向量
            if len(inds) == 0:
                continue
    ...
    def inference(self, x):
        ...
        # [B, H*W, 1]
        conf_pred = pred[..., :1] # 每个锚框的置信度
        # [B, H*W, num_cls]
        cls_pred = pred[..., 1:1+self.num_classes] # 每个类别的条件概率
        # [B, H*W, 4]
        txtytwth_pred = pred[..., 1+self.num_classes:] # 每个锚框的位置信息
    ...
    def forward(self, x, targets=None):
        ...
        if not self.trainable:
            ...
            # 对pred 的size做一些view调整，便于后续的处理
            # [B, C, H, W] -> [B, H, W, C] -> [B, H*W, C]
            pred = pred.permute(0, 2, 3, 1).contiguous().flatten(1, 2)  # 前向推断和预测的时候是一样的
            # 从pred中分离出objectness预测、类别class预测、bbox的txtytwth预测  
            # [B, H*W, 1]
            conf_pred = pred[..., :1]
            # [B, H*W, num_cls]
            ...
            # [B, H*W, 4]
            # 此时存储的应当是预测的锚框的中心点在当前grid_cell中的相对坐标，以及宽高的绝对长度
