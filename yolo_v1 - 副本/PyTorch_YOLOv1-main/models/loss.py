...
class MSEWithLogitsLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        inputs = torch.clamp(torch.sigmoid(logits), min=1e-4, max=1.0 - 1e-4)  # 求解出实际的置信度
...
def compute_loss(pred_conf, pred_cls, pred_txtytwth, targets):
    ...
    # 预测(grid_cell的个数就是H*W)，pred_conf=[B,HW]
    pred_conf = pred_conf[:, :, 0]           # [B, HW,] # 预测的置信度(取sigmoid后才是真正的置信度)
    pred_cls = pred_cls.permute(0, 2, 1)     # [B, C, HW] # 预测的每个类别的概率
    pred_txty = pred_txtytwth[:, :, :2]      # [B, HW, 2] # 预测的锚框在当前的grid_cell中的相对位置
    pred_twth = pred_txtytwth[:, :, 2:]      # [B, HW, 2] # 预测的锚框在当前的grid_cell中的绝对长度
    
    # 标签
    gt_obj = targets[:, :, 0]                  # [B, HW,]
    gt_cls = targets[:, :, 1].long()           # [B, HW,] # 当前grid_cell中的标签类别
    gt_txty = targets[:, :, 2:4]               # [B, HW, 2] # 标签框的中心坐标在当前的grid_cell中的相对位置
    gt_twth = targets[:, :, 4:6]               # [B, HW, 2] # 标签框宽高的绝对长度
    gt_box_scale_weight = targets[:, :, 6]     # [B, HW,] # 框的权重(框越大权重越小)

    batch_size = pred_conf.size(0)
    # 置信度损失
    conf_loss = conf_loss_function(pred_conf, gt_obj)  # grid_cell不包含标签框以及包含标签框的置信度损失
