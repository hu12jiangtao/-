    ...
    def __call__(self, target, width, height):
        ...
        for obj in target.iter('object'):
            ...
            for i, pt in enumerate(pts):
                ...
                cur_pt = cur_pt / width if i % 2 == 0 else cur_pt / height # 将绝对坐标转换为相对的坐标
        ...
        return res  # [[xmin, ymin, xmax, ymax, label_ind], ... ] 此时返回的内容为将要输出的这张图片的相对坐标以及类别标签
...
class VOCDetection(data.Dataset):
    ...
    def pull_item(self, index):
        img_id = self.ids[index]  # 其中存储这一个元组，第一个元素是存放路径的VOC2007数据集所在的路径的，第二个数据集是用来存放导出的图片的序列的
        target = ET.parse(self._annopath % img_id).getroot() # self._annopath % img_id用于确定当前标签的路径
        # img = cv2.imread(self._imgpath % img_id) # 此时根据img_id确定当前图片的路径，并读取这张图片
        ...
        if self.transform is not None:
            ...
            target = np.hstack((boxes, np.expand_dims(labels, axis=1))) # 将label和boxes信息贴在一起
