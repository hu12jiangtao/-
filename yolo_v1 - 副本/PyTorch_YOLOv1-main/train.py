...
def parse_args():
    ...
    parser.add_argument('--dataset', default='voc',
                        help='voc or coco')
    parser.add_argument('--root', default='D:\\python\\pytorch作业\\计算机视觉\\data',
                        help='data root')
...
def train():
    ...
    # 构建我们的模型
    model = build_yolo(args, device, train_size, num_classes=20, trainable=True)
    ...
    for epoch in range(args.start_epoch, max_epoch):
        ...
        for iter_i, (images, targets) in enumerate(dataloader):
            ...
            # 更新(此时的更新为经过args.accumulate次迭代后(进行args.accumulate次的梯度累加)才进行一次的梯度更新)
        ...
        if epoch  % args.eval_epoch == 0 or (epoch + 1) == max_epoch:
            ...
            cur_map = evaluator.map  # 求解其评估指标MAP
            if cur_map > best_map:
                # update best-map
                best_map = cur_map
                # save model
...
if __name__ == '__main__':
    torch.manual_seed(1) # 去掉随机种子的效果更好(收敛的速度更快)
    train()
