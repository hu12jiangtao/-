...
def load_cifir_data(path,train_batch_size,test_batch_size):
    ...
    train_trans = torchvision.transforms.Compose([torchvision.transforms.RandomCrop(32,padding=4),
                                                  torchvision.transforms.RandomHorizontalFlip(),
                                                  torchvision.transforms.ToTensor(),
                                                  torchvision.transforms.Normalize([0.4914, 0.4822, 0.4465],
                                                                                   [0.2023, 0.1994, 0.2010])])
    test_trans = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
                                                 torchvision.transforms.Normalize([0.4914, 0.4822, 0.4465],
                                                                                  [0.2023, 0.1994, 0.2010])])
...
def train_epoch(epoch,net,train_loader,test_loader,loss,opt,config):
    ...
    for index,(X,y) in enumerate(train_loader):
        ...
        if config.loss_mode == 1:
            l = loss(y_hat,y,config.smoothing)
        else:
            l = loss(y_hat,y)
        opt.zero_grad()
        l.backward()
...
if __name__ == '__main__':
    a = Image.open('buhuo.PNG')
    trans = torchvision.transforms.Compose([torchvision.transforms.RandomCrop(32,padding=2),
                                            torchvision.transforms.ToTensor()])
    print(trans(a))