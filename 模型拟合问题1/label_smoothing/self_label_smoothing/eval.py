...
def T_SNE(num_dim,inputs):
    t_sne = TSNE(n_components=num_dim,random_state=0,init='pca')
...
def save_plt(num_dim,title,down_mode):
    ...
    if title == 'no_smoothing':
        net.load_state_dict(torch.load('params/no_smoothing.pkl'))  # 此时用的是给定的的模型checkpoint/CrossEntropy.bin
    elif title == 'smoothing':
        net.load_state_dict(torch.load('params/smoothing.pkl')) # 此时用的是给定的的模型checkpoint/LabelSmoothing.bin
